    ...
    <div className="flex flex-col md:flex-row mx-auto py-6 lg:px-8">
      <aside className="w-full md:w-1/4 p-4 rounded-lg">
        ...
        <div className="md:hidden relative">
          <button onClick={() => setDropdownOpen(!dropdownOpen)} className="w-full text-left px-4 py-2 bg-white border rounded-md">
          ...
          {dropdownOpen && (
            <div className="absolute left-0 right-0 bg-white border mt-1 shadow-lg rounded-md z-10">
              {categorias.map((categoria) => (
                <button
                  key={categoria.id}
                  onClick={() => { setSelectedCategories([categoria.id]); setDropdownOpen(false); }}
